...
This module provides inference capabilities for vision-language models
using vLLM (server/local), OpenAI API, or OpenRouter API.
"""
...
# Optional PIL import for local vLLM mode
try:
...
class VLMInference:
    ...
    3. OpenAI: Use OpenAI API for inference
    4. OpenRouter: Use OpenRouter API for inference
    """
    ...
    def __init__(
        ...
        Args:
            ...
            api_base: API base URL (for vLLM server, OpenAI, or OpenRouter)
            api_key: API key (required for OpenAI/OpenRouter, "EMPTY" for local vLLM)
        ...
        if self.provider == "openrouter":
            ...
            logger.debug(f"Initialized vLLM server client: {api_base}")
        # Note: vLLM local mode is handled lazily in _ensure_local_model()
    ...
    async def _ensure_local_model(self):
        ...
        if self._llm is None and self.api_base is None:
            if not PIL_AVAILABLE:
                raise ImportError("PIL (Pillow) is required for local vLLM mode. Install with: pip install Pillow")
            try:
    ...
    async def generate(
        ...
        """
        if self.provider == "openrouter" or self.provider == "openai" or (self.provider == "vllm" and self.api_base):
    ...
    async def _generate_server(
        ...
        """Generate using server API (vLLM, OpenAI, or OpenRouter).
        ...
        try:
            ...
            provider_name = "OpenRouter" if self.provider == "openrouter" else ("OpenAI" if self.provider == "openai" else "vLLM")
